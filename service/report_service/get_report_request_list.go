...
func (handle *ReportService) GetReportRequestList(req mwsapi.Values) (rsp *mwsapi.ReportRequestListResult, err error) {
	report := mwsapi.Reports()
	ctx := context.Background()
	credential := setting.MwsSetting
	_, rsp, err = report.GetReportRequestList(ctx, credential, req)
	if err != nil {
		logging.Error(err)
		return
	}
